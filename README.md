...
| [`h3-ngnet-qmux`](crates/h3-ngnet-qmux) | Not published. Implements hyperium H3's per-stream QUIC traits over an established `ngnet-qmux` connection, with one caller-polled driver and no endpoint, TLS, runtime, or spawned task. |
| [`h3-ngnet-qmux-tests`](tests/h3-ngnet-qmux-tests) | Not published. Drives hyperium H3 over QMux end to end on non-`Send` memory streams and loopback TCP, including fragmentation, flow control, lifecycle, and portability. |
