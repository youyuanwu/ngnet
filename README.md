...
| [`ngnet-qmux-h3`](crates/ngnet-qmux-h3) | Not published yet, alongside the two crates it binds. HTTP/3 over QMux: implements `ngnet-h3`'s transport trait on an `ngnet-qmux` connection, so HTTP/3 runs over TCP, a unix socket or a TLS session — the draft's own stated motivation. The only crate that depends on both families, which a dependency-graph test enforces. |
| [`ngnet-qmux-h3-tests`](tests/ngnet-qmux-h3-tests) | Not published. Drives HTTP/3 over QMux end to end, in memory and across a real loopback TCP socket. |
| [`h3-ngnet-qmux`](crates/h3-ngnet-qmux) | Not published. Implements hyperium H3's per-stream QUIC traits over an established `ngnet-qmux` connection, with one caller-polled driver and no endpoint, TLS, runtime, or spawned task. Its dev-only matrix covers fragmented non-`Send` memory streams, lifecycle failures, and loopback TCP. |
| [`h3-ngnet-quic`](crates/h3-ngnet-quic) | Not published. The same join for the other transport: implements hyperium H3's per-stream QUIC traits over an established `ngnet-quic` connection, so the community HTTP/3 stack runs on ngtcp2. Deliberately exposes no driver future — the transport is driven from inside the trait methods H3 already calls, as `ngnet-quic-h3` does, and the connection's expiry timer is polled under a waker the adapter owns so a quiet connection still fires its own timers. Unlike the QMux adapter it reaches OpenSSL, because its transport does. An intermittent liveness failure found during development turned out to be a lost FIN one layer down — ngtcp2 distinguishes "no STREAM frame was written" from "a zero-length STREAM frame was written", which is what a `fin`-only offer produces, and `ngnet-quic` had clamped the two together; the transport now reports them separately and all of this crate's live tests are enabled. See [`docs/h3-ngnet-quic/pending-work.md`](docs/h3-ngnet-quic/pending-work.md). |
