...
#
# Both vendored libraries declare nested submodules that only their own tests, tooling and
# example applications use, so a `--recursive` checkout fetches several hundred megabytes
...
# is correct for nghttp2 and quietly wrong for nghttp3.
#
...
submodules depth="":
    ...
    git submodule update "${args[@]}" deps/nghttp2 deps/nghttp3
