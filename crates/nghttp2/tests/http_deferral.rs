...
use nghttp2::http::{Transport, TransportRead};
use nghttp2::{FrameType, Header, HeaderAction, HeaderCategory, Session, SessionBuilder, StreamId};
...
fn watched<M, C, O>(main: M, connection: C) -> (O, Option<nghttp2::http::Result<()>>)
where
    M: Future<Output = O>,
    C: Future<Output = nghttp2::http::Result<()>>,
{
    let outcome: Arc<Mutex<Option<nghttp2::http::Result<()>>>> = Arc::new(Mutex::new(None));
...
fn a_body_that_wakes_itself_does_not_deadlock() {
    let (client_side, server_side) = duplex(false);
    let (requests, connection) =
        nghttp2::http::handshake::<_, SelfWaking>(client_side).expect("handshake");
...
fn a_read_in_flight_does_not_block_a_write() {
    ...
    let (requests, connection) =
        nghttp2::http::handshake::<_, Scripted>(StalledRead(client_side)).expect("handshake");
