...
//!
//! Note this crate's `Cargo.toml` declares only `ngnet-h2-sys` as a runtime dependency
//! and this test does not name it at all; everything below goes through `ngnet_h2::raw`.
...
fn unwrapped_capability_is_reachable_through_the_escape_hatch() {
    ...
    let info = unsafe { ngnet_h2::raw::nghttp2_version(0) };
    assert!(!info.is_null());
...
fn raw_error_codes_translate_through_the_safe_error_type() {
    ...
    let error = ngnet_h2::Error::from_native("nghttp2_session_mem_recv2", ngnet_h2::raw::NGHTTP2_ERR_NOMEM);

    assert_eq!(error.kind(), ngnet_h2::ErrorKind::Exhausted);
