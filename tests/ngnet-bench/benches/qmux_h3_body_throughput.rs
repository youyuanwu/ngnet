//! Matched ngnet and hyperium H3 body round trips over QMux and Tokio duplex.

use std::hint::black_box;

use criterion::{BenchmarkId, Criterion, Throughput, criterion_group, criterion_main};
use ngnet_bench::{NgnetQmuxH3Matched, UpstreamH3Qmux, body_of, current_thread_runtime};

const SIZES: &[usize] = &[0, 1024, 64 * 1024, 1024 * 1024, 8 * 1024 * 1024];

fn qmux_h3_body_throughput(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("qmux_h3_body_throughput");
...
criterion_group!(benches, qmux_h3_body_throughput);
