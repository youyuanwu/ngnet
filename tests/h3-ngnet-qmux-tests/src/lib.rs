...
use h3::client;
use h3::server;
use h3_ngnet_qmux::{AdapterConfig, OpenStreams, from_qmux_with_config};
...
fn spawn_local_endpoint<S, C>(lower: QmuxConnection<S, C>) -> h3_ngnet_qmux::Connection<S, C, Bytes>
where
...
fn spawn_send_endpoint<S, C>(lower: QmuxConnection<S, C>) -> h3_ngnet_qmux::Connection<S, C, Bytes>
where
