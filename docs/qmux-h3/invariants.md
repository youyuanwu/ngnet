...
| **A stream ending keeps its own event batch** | `final_data_and_stream_close_are_separated_by_a_woken_boundary`. The transport trace is final `Data`, a self-woken `Pending`, then `StreamClosed`, so the close cannot overtake the last bytes. Response-head and body settlement remain covered end to end by `exchange.rs`, `multiplexing.rs`, and the concurrent write-count workload. |
