...
| **Shared work cannot be missed while the driver parks or completes** | Unit tests beside `crates/ngnet-h3/src/http/shared.rs` pin the coherent five-category drain and both sides of atomic waker registration/readiness recheck. `http/driver/shared_snapshot.rs` drives post-snapshot and action-induced work through a real server state machine, proving it takes a second pass and the peer decodes GOAWAY before completion. Run 15 supplies the QMux/H3 operation counts and controlled timing. |
