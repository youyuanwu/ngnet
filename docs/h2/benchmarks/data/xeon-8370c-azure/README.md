...
All eight bench targets. `cargo bench -p ngnet-h2-bench -- --test` completes with `Success` for
...
`ngnet-h2-bench` depends on `ngnet-h2` alone, so it needs neither the ngtcp2/nghttp3
submodules nor OpenSSL ≥ 3.5 that the QUIC crates require: `cargo bench -p ngnet-h2-bench`
