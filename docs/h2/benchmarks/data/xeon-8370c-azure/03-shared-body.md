...
**Command:** `taskset -c 3 cargo bench -p ngnet-h2-bench --bench transport_shared_body --
...
gain was 4.07%. `pending-work.md` states what that needs: *"a quieter machine and a
...
- **SC-005 is MET on the completion transport.** −4.55% at 1 MiB against `compio-push`'s own
  ...
  misbehaving control rather than on its own delta — precisely the outcome `pending-work.md`
  said a quieter machine might produce. The gain is small, and it is a gain.
...
- **The replicate count was not pre-registered.** Two passes were run as part of
  ...
  first two agreed. That is the letter of what `pending-work.md` asked for left unmet, and it
  is recorded rather than glossed. What mitigates it: no replicate was discarded, the
