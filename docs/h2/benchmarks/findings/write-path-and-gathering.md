...
> here and in `design.md`. Emulated gathering accumulates in the driver *first*, so the small
> blocks collapse into one region before the emulating loop sees them; that is why
