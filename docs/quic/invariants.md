...
| `the_allowance_list_is_the_ffi_boundary_and_nothing_else` | That list is exactly the fourteen modules that touch the raw bindings. The compiler cannot express this — adding an allowance is precisely how the crate-level deny would be silenced — so changing it must be a deliberate edit in two places. The asynchronous subtree adds no entry, and the test below says why it needs none. |
