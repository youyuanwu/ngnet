...
| **A retained send datagram is copied once** | The driver writes each datagram into one reusable send buffer and sends straight from it (`flush`, `next_datagram`, `write_stream` in `src/endpoint/driver.rs`), so a datagram the socket accepts is not copied. Only a datagram the socket **refuses** — one held as `tracked.pending` until a later pass — is copied into a buffer of its own, because a later write in the same pass overwrites the shared buffer. **What would settle it:** nothing removes it — a datagram retained past the buffer's reuse must be owned; a per-connection buffer would relocate the allocation, not remove it. Pinned by `a_completing_send_pass_allocates_nothing` and `a_core_produced_datagram_costs_nothing_to_send_and_one_to_retain`, which assert zero on send and one on retain. |
