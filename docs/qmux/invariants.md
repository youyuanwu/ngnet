...
| Property | Where |
| --- | --- |
| **A record reaches the byte stream in full before the next one is produced** | `io_lifecycle.rs`, `a_backed_up_transport_does_not_lose_a_record`, against a byte stream that accepts one byte per call — the least generous transport a connection can be handed. It asserts the record arrives *whole and unduplicated*, which is the pair of failures a partial accept invites: resuming from the wrong offset, and resending what was already taken. `io_transfer.rs`'s `a_transfer_survives_one_byte_per_call` is the same discipline over a whole exchange. The memory consequence — one record outstanding rather than the caller's whole backlog — follows from producing only into an empty buffer and is not separately measured. |
...
| **The three ways a byte stream can stop are distinguishable** | `io_lifecycle.rs`: `a_failing_byte_stream_ends_the_connection`, `a_byte_stream_that_ends_between_records_reports_the_end_of_the_stream`, and `a_byte_stream_that_ends_mid_record_reports_a_truncated_record`. The middle one is the ending a peer that simply exits produces, pinned again at the connection level by `a_peer_that_disappears_without_closing_is_an_ordinary_ending`; the last is the one dwnx cannot tell from it, and reporting a truncation as a clean ending is an incomplete transfer nobody notices. |
