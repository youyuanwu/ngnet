...
| Invariant | Evidence |
| --- | --- |
| One lower read batch and at most 64 routed events per adapter turn | `ngnet-qmux/tests/io_bounded.rs`; `h3-ngnet-qmux/tests/bounds.rs` |
...
| Generic framed chunks and borrowed unframed buffers advance exactly | `tests/send_buffers.rs` |
| Delivery credits stream and connection exactly once; discard credits connection only | `tests/lifecycle.rs`; `tests/diagnostics.rs` |
| Finish/reset/stop/drop and repeated terminal polls are idempotent | `tests/lifecycle.rs` |
| Late data beyond a routing budget cannot resurrect a reclaimed stream | `tests/bounds.rs` |
