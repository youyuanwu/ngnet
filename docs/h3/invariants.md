...
| `crates/ngnet-h3-tests/tests/http_quinn.rs` | The same exchanges over a real QUIC connection, driven by a second, independent implementation of the backend trait. It declares `RETAINS_BUFFERS = false` where the in-memory one declares `true`, so both arms of that constant are exercised. |
