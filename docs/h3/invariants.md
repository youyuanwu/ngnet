...
| Unit tests beside `http/driver.rs` and `tests/http_closed_streams.rs` | Closed-stream membership and FIFO order stay synchronized across insertion, duplicate notification and eviction, with exactly 1,024 retained entries. The driver discards the state machine's callback observation for a close it has already applied, so a batch larger than the tombstone window cannot replay side effects after eviction. A release for a retained tombstone is discarded and the connection keeps working; the same nonzero delivered release after eviction follows the unchanged non-tombstoned error path. |
| `tests/http_flush.rs` | Before the connection future can suspend on binding, request-stream opening, transmit backpressure, or its idle event poll, it polls the required transport flush operation. Call ordering, pending-with-wake continuation, and explicit-error behavior cover all four boundaries. There is no default trait implementation that can silently waive the contract. |
| Unit tests beside `http/shared.rs` and `http/driver/shared_snapshot.rs` | One drain preserves FIFO ready/reset/action ordering, duplicate suppression, credit aggregation and one-shot shutdown across all five categories. Work arriving after a drain stays for the next snapshot; a transferred batch is not replayed after processing stops. The park recheck covers both sides of waker registration, and a driven server proves post-snapshot and action-induced shutdown take a second pass, emit GOAWAY, and use one drain/completion probe per pass. Scratch-capacity tests cross both hysteresis thresholds. |
...
| `tests/ngnet-h3-tests/tests/http_quinn.rs` | The same exchanges over a real QUIC connection, driven by the independent implementation in `ngnet-h3-quinn`. It declares `RETAINS_BUFFERS = false` where the in-memory one declares `true`, so both arms of that constant are exercised. |
| `crates/ngnet-h3-quinn/src/lifecycle.rs` unit tests and `tests/ngnet-h3-tests/tests/http_quinn.rs` | Quinn bidirectional closure is emitted exactly once after both directions terminate. Final FIN/reset and every accepted-byte release for that stream precede closure across a `Pending` batch boundary; unrelated releases cannot starve it. Late terminal observations preserve the first direction outcome, connection shutdown emits one final `Closed`, 1,000 synthetic completed streams leave no retained bidirectional ownership, and 1,000 sequential real streams each close once after their own FIN. Live cancellation reaches Quinn as both `RESET_STREAM` and `STOP_SENDING`, including when receive credit is parked at zero. |
