...
| `tests/http_flush.rs` | Before the connection future can suspend on binding, request-stream opening, transmit backpressure, or its idle event poll, it polls the required transport flush operation. Ready, pending-with-wake, and explicit-error outcomes are covered at each boundary; there is no default trait implementation that can silently waive the contract. |
