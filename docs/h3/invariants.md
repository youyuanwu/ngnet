...
| `tests/http_flush.rs` | Before the connection future can suspend on binding, request-stream opening, transmit backpressure, or its idle event poll, it polls the required transport flush operation. The ready case covers all four boundaries; pending-with-wake and explicit-error behavior are each covered on the representative bind path. There is no default trait implementation that can silently waive the contract. |
