...
| Unit tests beside `http/shared.rs` | One drain preserves FIFO ready/reset/action ordering, duplicate suppression, credit aggregation and one-shot shutdown across all five categories. Work arriving after a drain stays for the next snapshot; a transferred batch is not replayed after processing stops. The park recheck covers both sides of waker registration, operation-class counters pin the consolidated entry points, and scratch-capacity tests cross both hysteresis thresholds. |
