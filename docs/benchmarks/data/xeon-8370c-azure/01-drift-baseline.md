...
```sh
cargo build --benches -p ngnet-bench --release   # pre-built, so compilation never contends
taskset -c 3 cargo bench -p ngnet-bench -- --save-baseline r1
taskset -c 3 cargo bench -p ngnet-bench -- --save-baseline r2
```
...
> **Editorial note, 2026-08-17.** The commands above name the benchmark crate as it is
> called now. At commit `e75118e` it was still named for HTTP/2 alone; it was renamed to
> `ngnet-bench` when the suite stopped being an HTTP/2-only one, and the commands were
> corrected here so that they still run. They are otherwise the ones that were run, and
> nothing below them has been touched.
