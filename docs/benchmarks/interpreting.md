...
Feature-gated `h3-ngnet-qmux` counters are asymmetric focused evidence only; the baseline
adapter exposes no matching units, so those counts cannot attribute a timing delta.
